...
thread_start (void) 
{
  ...
  /* Initialize load_avg, then start the interrupt. */
  load_avg = FP_CONST (0);
...
/* Implement the compare func for the list_insert_ordered. */
bool
list_sleep_less(const struct list_elem *a,
...
thread_mlfqs_calc_recent_cpu(struct thread *t)
{
  ...
  fixed_t temp1 = FP_DIV (FP_MULT_MIX (load_avg, 2),
             FP_ADD_MIX (FP_MULT_MIX (load_avg, 2), 1));
  fixed_t temp2 = FP_MULT (temp1, t->recent_cpu);
  t->recent_cpu = FP_ADD_MIX (temp2, t->nice);
}
